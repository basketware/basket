...
size_t string_len(const char *string, size_t max) {
    size_t length = 0;
...
#undef CHANGED
	...
	if ((g_flags & TFX_RESET_DEBUG_OVERLAY_STATS) == TFX_RESET_DEBUG_OVERLAY_STATS) {
		...
		for (unsigned i = 0; i < stats.num_timings; i++) {
			int len = string_len(stats.timings[i].name, 100);
			if (len > max_width) {
				max_width = len;
			}
		}
		...
		for (unsigned i = 0; i < stats.num_timings; i++, row++) {
			...
			tfx_debug_print(row, width - string_len(str, 100), color[row % 2], 0, str);
			...
			tfx_debug_print(row, 8 + width - string_len(str, 20), color[row % 2], 0, str);
			free((char*)str);
		}
		tfx_debug_print(row, 0, 0x100f, 0, "Total:");
		str = tfx_sprintf("%dus", sum / 1000);
		tfx_debug_print(row, 13 + max_width - string_len(str, 20), 0x100f, 0, str);
		free((char*)str);
	}
